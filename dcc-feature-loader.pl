...
# This method takes a hash generated by parseMySQLSchema, a table name and a list of columns,
# and it returns the column positions (needed for MySQL dumps)
sub mapTableColumns(\%$@) {
	my($p_ensTables,$tableName,@columnNames)=@_;
	
	my @colpos = ();
	
	if(exists($p_ensTables->{$tableName})) {
		my $p_table = $p_ensTables->{$tableName};
...
if(scalar(@ARGV)>=2) {
	...
	my $p_ensTables = parseMySQLSchema($localSQLFile);
	...
	if(open(my $SEQREG,'-|',BP::Loader::Tools::GUNZIP,'-c',$localSeqRegion)) {
		my %config = (
			...
			#	seq_region_id
			#	name
			TabParser::TAG_FETCH_COLS => mapTableColumns(%{$p_ensTables},'seq_region',
				'seq_region_id',
				'name'
			), # [0,1],
		);
	...
	if(open(my $ENSG,'-|',BP::Loader::Tools::GUNZIP,'-c',$localGenes)) {
		my %config = (
			...
			#	description
			#	internal gene id
			TabParser::TAG_FETCH_COLS => mapTableColumns( %{$p_ensTables}, 'gene',
				'seq_region_id',
				'seq_region_start',
				'seq_region_end',
				'display_xref_id',
...
#			TabParser::TAG_NEG_FILTER => [[1 => 'LRG_gene']],
	...
	if(open(my $ENST,'-|',BP::Loader::Tools::GUNZIP,'-c',$localTranscripts)) {
		my %config = (
			...
			#	internal gene id
			TabParser::TAG_FETCH_COLS => mapTableColumns( %{$p_ensTables}, 'transcript',
				'seq_region_id',
				'seq_region_start',
				'seq_region_end',
				'display_xref_id',
...
#			TabParser::TAG_NEG_FILTER => [[8 => 'LRG_gene']],
	...
	if(open(my $XREF,'-|',BP::Loader::Tools::GUNZIP,'-c',$localXref)) {
		my %config = (
			...
			#	display_label
			#	description
			TabParser::TAG_FETCH_COLS => mapTableColumns( %{$p_ensTables}, 'xref',
				'xref_id',
				'dbprimary_acc',
				'display_label',
				'description'
